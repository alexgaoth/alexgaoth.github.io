...
// ── Island export ─────────────────────────────────────────────────
// Hydrates over the static shell rendered by now.astro: once mounted it
// removes #now-static so no-JS visitors (and crawlers) keep the static
// data while JS visitors get the interactive board.
const NowBoard = ({ initialNow = NOW_FALLBACK }) => {
  const [now, setNow] = React.useState(initialNow);

  React.useEffect(() => {
    document.getElementById("now-static")?.remove();
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: NB.paper }}>
      <Board onBack={() => { window.location.href = "/"; }} now={now} />
    </div>
  );
};
